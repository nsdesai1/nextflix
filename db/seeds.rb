...
video2 = Video.create({title: "Star Wars: The Force Awakens", description: "Three decades after the Empire's defeat, a new threat arises in the militant First Order. Defected stormtrooper Finn and the scavenger Rey are caught up in the Resistance's search for the missing Luke Skywalker.", media_type: "movie", duration: "2h 15m", rating: "PG-13", year: "2015"})
    video2.trailer.attach(io: open("https://nextflix-seeds.s3-us-west-1.amazonaws.com/the_force_awakens.mp4"), filename: "the_force_awakens.mp4")
    video2.thumbnail.attach(io: open("https://nextflix-seeds.s3-us-west-1.amazonaws.com/the_force_awakens_thumbnail.jpg"), filename:"the_force_awakens.jpg")

video3 = Video.create({title: "Arrival", description: "A linguist works with the military to communicate with alien lifeforms after twelve mysterious spacecraft appear around the world.", media_type: "movie", duration: "1h 56m", rating: "PG-13", year: "2016"})
    video3.trailer.attach(io: open("https://nextflix-seeds.s3-us-west-1.amazonaws.com/arrival.mp4"), filename: "arrival.mp4")
    video3.thumbnail.attach(io: open("https://nextflix-seeds.s3-us-west-1.amazonaws.com/arrival.jpg"), filename: "arrival.jpg")

video4 = Video.create({title: "Ex Machina", description: "A young programmer is selected to participate in a ground-breaking experiment in synthetic intelligence by evaluating the human qualities of a highly advanced humanoid A.I.", media_type: "movie", duration: "1h 48m", rating: "R", year: "2015"})
    video4.trailer.attach(io: open("https://nextflix-seeds.s3-us-west-1.amazonaws.com/ex_machina.mp4"), filename: "ex_machina.mp4")
    video4.thumbnail.attach(io: open("https://nextflix-seeds.s3-us-west-1.amazonaws.com/ex_machina.jpg"), filename: "ex_machina.jpg")




video5 = Video.create({title: "The Grand Budapest Hotel", description: "A hotel concierge recounts his adventures at a famous European hotel, including the theft and recovery of a priceless Renaissance painting and the battle for an enormous family fortune.", media_type: "movie", duration: "1h 40m", rating: "R", year: "2014"})
    video5.trailer.attach(io: open("https://nextflix-seeds.s3-us-west-1.amazonaws.com/the_grand_budapest_hotel.mp4"), filename: "the_grand_budapest_hotel.mp4")
    video5.thumbnail.attach(io: open("https://nextflix-seeds.s3-us-west-1.amazonaws.com/grand_budapest_hotel_thumbnail.jpg"), filename:"grand_budapest_hotel_thumbnail.jpg")

video6 = Video.create({title: "Parasite", description: "A poor family, the Kims, con their way into becoming the servants of a rich family, the Parks. But their easy life gets complicated when their deception is threatened with exposure.", media_type: "movie", duration: "2h 12m", rating: "R", year: "2019"})
    video6.trailer.attach(io: open("https://nextflix-seeds.s3-us-west-1.amazonaws.com/parasite.mp4"), filename: "parasite.mp4")
    video6.thumbnail.attach(io: open("https://nextflix-seeds.s3-us-west-1.amazonaws.com/parasite.jpg"), filename: "parasite.jpg")

video7 = Video.create({title: "Knives Out", description: "When renowned crime novelist Harlan Thrombey is found dead at his estate just after his 85th birthday, the inquisitive and debonair Detective Benoit Blanc is mysteriously enlisted to investigate.", media_type: "movie", duration: "2h 11m", rating: "PG-13", year: "2019"})
    video7.trailer.attach(io: open("https://nextflix-seeds.s3-us-west-1.amazonaws.com/knives_out.mp4"), filename:"knives_out.mp4")
    video7.thumbnail.attach(io: open("https://nextflix-seeds.s3-us-west-1.amazonaws.com/knives_out.jpg"), filename:"knives_out.jpg")

video8 = Video.create({title: "The Wolf of Wall Street", description: "From the American dream to corporate greed, New York stockbroker Jordan Belfort goes from penny stocks and righteousness to IPOs and a life of corruption in the late 80s.", media_type: "movie", duration: "2h 59m", rating: "R", year: "2013"})
